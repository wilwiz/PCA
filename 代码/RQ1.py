...
def draw(data,plot_title):
    ...
    colors = ['orange', 'goldenrod','gold','olive','yellow',
              'palegreen','yellowgreen','green','seagreen','springgreen',
              'turquoise','teal','c','lightblue','dodgerblue',
              'darkviolet','hotpink','pink','brown','red','','',]
    for patch, color in zip(bplot['boxes'], colors):
        patch.set_facecolor(color)

    plt.xticks([y + 1 for y in range(len(tang_array))], [str(i+1) for i in range(len(data))])
...
if __name__ == '__main__':
    ...
    for n_component in range(1,21):
        print("=================运行中，主元数量：",n_component)
        recall_list=[]; f1_list=[]; auc_list=[]
        for dcp_name in dcp_names:
            for clf_name in clf_names:
                ...
                for filename in file_list:
                    ...
                    # print(dcp_name,n_component, c,recall, f1, auc)
